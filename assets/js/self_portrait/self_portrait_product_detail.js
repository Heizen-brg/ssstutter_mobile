  ...
  variation(params = {}) {
    ...
    let init_add_to_cart = (params) => {
      ...
      to_cart_btn.addEventListener("click", (e) => {
        document.body.appendChild(pre_order_cart());
        let cart_selected = JSON.parse(localStorage.getItem("self-portrait-item"))
          ? JSON.parse(localStorage.getItem("self-portrait-item"))
          : [];
        ...
        if (product_in_cart) {
          ...
          localStorage.setItem("self-portrait-item", JSON.stringify(cart_selected));
        } else {
